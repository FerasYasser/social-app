...
<body>
  ...
  {
      require 'dbhandler.php';
      $search = $_SESSION['emailsearch'];
      echo $search."<br>";
      $email1 = $_SESSION['email'];
      echo $email1."<br>";
  $query = "SELECT  email1 From `friends` WHERE (email1='$email1' AND email2='$search' ) OR (email1='$search' AND email2='$email1');";
  $q = mysqli_query($connect,$query);
  $result=mysqli_num_rows($q);
    if($result==0) {echo "friend request sent";
    $query =  "INSERT INTO `friends` (`email1`,`email2`) VALUES('$email1','$search');";
      $q = mysqli_query($connect,$query);
}  if($result ==1 )
  {
